(function () {
  ...
  angular.module('BlurAdmin.pages.maps')
    .directive('gmapReact', ['baConfig', 'layoutPaths', function (baConfig, layoutPaths) {
      return {
        restrict: 'E',
    ...
    .directive('leafletReact', ['baConfig', 'layoutPaths', function (baConfig, layoutPaths) {
      return {
        restrict: 'E',
        link: function (scope, element) {
          var lib = window.BlurAdminReact;
          if (!lib || !lib.LeafletPage) {
            element.html('<p class="text-danger">React bundle not loaded.</p>');
            return;
          }
          var root = lib.mountReactApp(lib.LeafletPage, element[0], {
            baConfig: baConfig,
            layoutPaths: layoutPaths
          });
          scope.$on('$destroy', function () {
