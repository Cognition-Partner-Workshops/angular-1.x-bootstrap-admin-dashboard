...
function useAmChart(configFn) {
  var ref = useRef(null);
  var chartRef = useRef(null);

  useEffect(function () {
    if (!ref.current) return;
    var id = 'amchart-' + Math.random().toString(36).substr(2, 9);
    ref.current.setAttribute('id', id);
    chartRef.current = configFn(id);
    return function () {
...
function BarChart({ layoutColors }) {
  var ref = useAmChart(function (id) {
    return AmCharts.makeChart(id, {
      ...
      pathToImages: LAYOUT_PATHS.amChart
    });
  });
