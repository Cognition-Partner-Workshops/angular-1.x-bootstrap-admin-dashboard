/**
 * DashboardPage — React replacement for the AngularJS dashboard module.
 *
 * Receives `baConfig` as a prop (injected via react2angular bridge directive).
 * Initialises jQuery plugins (easyPieChart, Chart.js, AmCharts, FullCalendar)
 * inside useEffect hooks with proper cleanup.
 */
...
function DashboardPieCharts({ baConfig }) {
  ...
  return React.createElement('dashboard-pie-chart', null,
    React.createElement('div', { className: 'row pie-charts', ref: containerRef },
      charts.map(function (chart, i) {
        return React.createElement('div', { className: 'pie-chart-item-container', key: i },
          React.createElement(Panel, null,
            React.createElement('div', { className: 'pie-chart-item' },
              React.createElement('div', { className: 'chart', rel: chart.color, 'data-percent': '60' },
...
function TrafficChart({ baConfig }) {
  ...
  var channelsClass = 'channels-block' + (transparent ? ' transparent' : '');

  return React.createElement('div', { className: channelsClass },
    React.createElement('div', { className: 'chart-bg' }),
    React.createElement('div', { className: 'traffic-chart', id: 'trafficChart' },
      React.createElement('div', { className: 'canvas-holder' },
        React.createElement('canvas', { id: 'chart-area', width: 280, height: 280, ref: canvasRef }),
        React.createElement('div', { className: 'traffic-text' },
          '1,900,128',
          React.createElement('span', null, 'Views Total')
        )
      )
    ),
    React.createElement('div', { className: 'channels-info' },
      React.createElement('div', null,
        doughnutData.labels.map(function (label, i) {
          var data = doughnutData.datasets[0];
          return React.createElement('div', { className: 'channels-info-item', key: i },
            React.createElement('div', { className: 'legend-color', style: { backgroundColor: data.backgroundColor[i] } }),
            React.createElement('p', null,
              label,
              React.createElement('span', { className: 'channel-number' }, '+' + data.percentage[i] + '%')
            ),
            React.createElement('div', { className: 'progress progress-sm channel-progress' },
...
/* ---------- Dashboard Map ---------- */
function DashboardMap({ baConfig }) {
  var layoutColors = baConfig.colors;
  var mapRef = useRef(null);

  useEffect(function () {
    if (!window.AmCharts) return;
    var chart = window.AmCharts.makeChart('amChartMap', {
      type: 'map',
      theme: 'blur',
  ...
  return React.createElement('div', { id: 'amChartMap' });
}

/* ---------- Line Chart (Revenue) ---------- */
function DashboardLineChart({ baConfig }) {
  var layoutColors = baConfig.colors;
  ...
  return React.createElement('div', { id: 'amchart' });
}

/* ---------- Popular App ---------- */
function PopularApp() {
  return React.createElement('popular-app', null,
    React.createElement('div', { className: 'popular-app-img-container' },
      React.createElement('div', { className: 'popular-app-img' },
        React.createElement('img', { src: IMAGES_ROOT + 'app/my-app-logo.png' }),
        React.createElement('span', { className: 'logo-text' }, 'Super\u00a0App')
      )
    ),
    React.createElement('div', { className: 'popular-app-cost row' },
      React.createElement('div', { className: 'col-xs-9' }, 'Most Popular App'),
      React.createElement('div', { className: 'col-xs-3 text-right' }, '175$')
    ),
    React.createElement('div', { className: 'popular-app-info row' },
...
var FEED_DATA = [
  {
    ...
    time: 'Today 11:55 pm', ago: '25 minutes ago',
  },
  {
    ...
    time: 'Today 9:30 pm', ago: '3 hrs ago',
  },
  {
  ...
  },
  {
    ...
    time: '12.11.2015', ago: '3 days ago',
  },
  {
    ...
    time: '14.11.2015', ago: '5 days ago',
  },
  {
    type: 'text-message', author: 'Nasta', surname: 'Linnie',
    header: 'Posted new message',
  ...
  },
  {
    ...
    time: '15.11.2015', ago: '6 days ago',
  },
  {
    ...
    time: '16.11.2015', ago: '7 days ago',
  },
  {
    type: 'text-message', author: 'Kostya', surname: 'Danovsky',
    header: 'Posted new message',
    text: "Why did the CoffeeScript developer keep getting lost? Because he couldn't find his source without a map",
    time: '18.11.2015', ago: '9 days ago',
  },
];

function BlurFeed() {
  var initExpanded = FEED_DATA.map(function () { return false; });
  var expandedState = useState(initExpanded);
  var expanded = expandedState[0];
  var setExpanded = expandedState[1];

  var toggleMessage = useCallback(function (index) {
  ...
  return React.createElement('div', { className: 'feed-messages-container' },
    FEED_DATA.map(function (message, i) {
      var isExpanded = expanded[i];
      var isTextMessage = message.type === 'text-message';

      return React.createElement('div', {
        className: 'feed-message', key: i,
        onClick: function () { toggleMessage(i); },
      },
        React.createElement('div', { className: 'message-icon' },
          React.createElement('img', { className: 'photo-icon', src: profilePicture(message.author) }),
          !isTextMessage
            ? React.createElement('span', { className: 'sub-photo-icon ' + message.type })
            : null
        ),
        React.createElement('div', { className: 'text-block text-message' },
          React.createElement('div', { className: 'message-header' },
            React.createElement('span', { className: 'author' }, message.author + ' ' + message.surname)
          ),
          React.createElement('div', {
            className: 'message-content line-clamp' + (isExpanded ? '' : ' line-clamp-2'),
          },
            message.preview
              ? React.createElement('span', null, message.header + ' ')
              : null,
            message.text
          ),
          message.preview
            ? React.createElement('div', {
                ...
                React.createElement('a', { href: message.link, target: '_blank' },
                  React.createElement('img', { src: IMAGES_ROOT + message.preview })
                )
              )
            : null,
          React.createElement('div', {
            ...
            React.createElement('div', { className: 'post-time' }, message.time),
            React.createElement('div', { className: 'ago-time' }, message.ago)
          )
        )
      );
    })
...
function DashboardTodo({ baConfig }) {
  ...
  var newTodoText = inputState[0];
  var setNewTodoText = inputState[1];
  ...
  var containerClass = 'task-todo-container' + (transparent ? ' transparent' : '');
          ...
          onDelete: function () {
            setTodoList(function (prev) {
              return prev.map(function (t, idx) {
                if (t === item) return { text: t.text, color: t.color, deleted: true };
                return t;
              });
            });
          },
...
function TodoItem({ item, onDelete }) {
  var checkedState = useState(false);
  var isChecked = checkedState[0];
  var setIsChecked = checkedState[1];
  var hoverState = useState(false);
  var activeItem = hoverState[0];
  var setActiveItem = hoverState[1];

  var liClass = (isChecked ? 'checked' : '') + (activeItem ? ' active' : '');

  return React.createElement('li', {
    className: liClass.trim() || undefined,
    onMouseEnter: function () { setActiveItem(true); },
    onMouseLeave: function () { setActiveItem(false); },
  },
    React.createElement('div', { className: 'blur-container' },
      React.createElement('div', { className: 'blur-box' })
    ),
    React.createElement('i', { className: 'mark', style: { backgroundColor: item.color } }),
    React.createElement('label', { className: 'todo-checkbox custom-checkbox custom-input-success' },
      React.createElement('input', {
        type: 'checkbox',
        checked: isChecked,
        onChange: function (e) { setIsChecked(e.target.checked); },
      }),
...
function DashboardCalendar({ baConfig }) {
  ...
  useEffect(function () {
    var $ = window.jQuery;
    if (!$ || !calRef.current || !$.fn.fullCalendar) return;
    var $el = $(calRef.current);
    $el.fullCalendar({
      header: {
  ...
  return React.createElement('div', { id: 'calendar', className: 'blurCalendar', ref: calRef });
}

/* ---------- Dashboard Page (main) ---------- */
export function DashboardPage({ baConfig }) {
  if (!baConfig) return React.createElement('p', { className: 'text-danger' }, 'baConfig not available.');

  return React.createElement('div', null,
    React.createElement(DashboardPieCharts, { baConfig: baConfig }),

    React.createElement('div', { className: 'row' },
      React.createElement('div', { className: 'col-lg-6 col-md-12 col-sm-12' },
        React.createElement(Panel, { title: 'Acquisition Channels', panelClass: 'medium-panel traffic-panel' },
          React.createElement(TrafficChart, { baConfig: baConfig })
        )
      ),
      React.createElement('div', { className: 'col-lg-6 col-md-12 col-sm-12' },
        React.createElement(Panel, { title: 'Users by Country', panelClass: 'medium-panel' },
          React.createElement(DashboardMap, { baConfig: baConfig })
        )
      )
    ),

    React.createElement('div', { className: 'row' },
      React.createElement('div', { className: 'col-xlg-9 col-lg-6 col-md-6 col-sm-12 col-xs-12' },
        React.createElement('div', { className: 'row' },
          React.createElement('div', { className: 'col-xlg-8 col-lg-12 col-md-12 col-sm-7 col-xs-12' },
            React.createElement(Panel, { title: 'Revenue', panelClass: 'medium-panel' },
              React.createElement(DashboardLineChart, { baConfig: baConfig })
            )
          ),
          React.createElement('div', { className: 'col-xlg-4 col-lg-12 col-md-12 col-sm-5 col-xs-12' },
            React.createElement(Panel, { panelClass: 'popular-app medium-panel' },
              React.createElement(PopularApp, null)
            )
          )
        )
      ),
      React.createElement('div', { className: 'col-xlg-3 col-lg-6 col-md-6 col-sm-12 col-xs-12' },
        React.createElement(Panel, { title: 'Feed', panelClass: 'large-panel with-scroll feed-panel' },
          React.createElement(BlurFeed, null)
        )
      )
    ),

    React.createElement('div', { className: 'row shift-up' },
      React.createElement('div', { className: 'col-xlg-3 col-lg-6 col-md-6 col-xs-12' },
        React.createElement(Panel, { title: 'To Do List', panelClass: 'xmedium-panel feed-comply-panel with-scroll todo-panel' },
          React.createElement(DashboardTodo, { baConfig: baConfig })
        )
      ),
      React.createElement('div', { className: 'col-xlg-6 col-lg-6 col-md-6 col-xs-12' },
        React.createElement(Panel, { title: 'Calendar', panelClass: 'xmedium-panel feed-comply-panel with-scroll calendar-panel' },
