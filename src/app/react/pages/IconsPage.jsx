/**
 * IconsPage — migrated from src/app/pages/ui/icons (IconsPageCtrl.js + icons.html).
 *
 * The original used $scope.icons data arrays rendered via ng-repeat inside
 * ba-panel widgets. Here the same data drives React .map() output inside
 * <BaPanel> wrappers.
 */
...
var kameleonIcons = [
  { name: 'Beach', img: 'Beach' }, { name: 'Bus', img: 'Bus' },
  { name: 'Cheese', img: 'Cheese' }, { name: 'Desert', img: 'Desert' },
  { name: 'Images', img: 'Images' }, { name: 'Magician', img: 'Magician' },
  { name: 'Makeup', img: 'Makeup' }, { name: 'Programming', img: 'Programming' },
  { name: 'Shop', img: 'Shop' }, { name: 'Surfer', img: 'Surfer' },
  { name: 'Phone Booth', img: 'Phone-Booth' }, { name: 'Ninja', img: 'Ninja' },
  { name: 'Apartment', img: 'Apartment' }, { name: 'Batman', img: 'Batman' },
  { name: 'Medal', img: 'Medal-2' }, { name: 'Money', img: 'Money-Increase' },
  { name: 'Street View', img: 'Street-View' }, { name: 'Student', img: 'Student-3' },
...
function seeAll(href, text) {
  return React.createElement('a', { href: href, target: '_blank', className: 'see-all-icons' }, text);
}

function KameleonWidget() {
  return React.createElement(React.Fragment, null,
    React.createElement('div', { className: 'row clearfix' },
      kameleonIcons.map(function (icon) {
        return React.createElement('div', { className: 'kameleon-row', key: icon.name },
          React.createElement('div', { className: 'kameleon-icon' },
            React.createElement('img', { src: kameleonImg(icon.img) }),
            React.createElement('span', null, icon.name)
          )
        );
      })
    ),
    seeAll('http://www.kameleon.pics/', 'See all Kamaleon icons')
  );
}

function KameleonRoundedWidget() {
  return React.createElement(React.Fragment, null,
    React.createElement('div', { className: 'row clearfix' },
      kameleonRoundedIcons.map(function (icon) {
        return React.createElement('div', { className: 'kameleon-row', key: icon.name },
          React.createElement('div', { className: 'kameleon-icon with-round-bg ' + icon.color },
            React.createElement('img', { src: kameleonImg(icon.img) }),
            React.createElement('span', null, icon.name)
          )
        );
...
function IconListWidget({ rowClass, icons, render, href, text }) {
  return React.createElement(React.Fragment, null,
    React.createElement('div', { className: rowClass },
      icons.map(function (icon, i) {
        return React.createElement('div', { className: 'col-xs-2', key: i }, render(icon));
      })
    ),
    seeAll(href, text)
  );
  ...
  return React.createElement('div', { className: 'widgets' },
    React.createElement('div', { className: 'row' },
      React.createElement('div', { className: 'col-md-6' },
        React.createElement(BaPanel, { title: 'Kameleon SVG Icons', panelClass: 'with-scroll' },
          React.createElement(KameleonWidget)),
        React.createElement(BaPanel, { title: 'Socicon', panelClass: 'with-scroll' },
          React.createElement(IconListWidget, {
            rowClass: 'row icons-list danger',
            icons: socicon,
            render: function (icon) { return React.createElement('i', { className: 'socicon' }, icon); },
            href: 'http://www.socicon.com/chart.php',
            text: 'See all Socicon icons',
          }))
      ),
      React.createElement('div', { className: 'col-md-6' },
        React.createElement(BaPanel, { title: 'Icons With Rounded Background', panelClass: 'with-scroll' },
          React.createElement(KameleonRoundedWidget)),
        React.createElement(BaPanel, { title: 'ionicons', panelClass: 'with-scroll' },
          React.createElement(IconListWidget, {
            rowClass: 'row icons-list primary',
            icons: ionicons,
            render: function (icon) { return React.createElement('i', { className: icon }); },
            href: 'http://ionicons.com/',
            text: 'See all ionicons icons',
          })),
        React.createElement(BaPanel, { title: 'Font Awesome Icons', panelClass: 'with-scroll' },
          React.createElement(IconListWidget, {
            rowClass: 'row icons-list success awesomeIcons',
            icons: fontAwesomeIcons,
            render: function (icon) { return React.createElement('i', { className: icon }); },
            href: 'http://fortawesome.github.io/Font-Awesome/icons/',
            text: 'See all Font Awesome icons',
          }))
      )
    )
  );
