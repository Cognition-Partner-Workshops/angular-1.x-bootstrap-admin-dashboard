/**
 * AmChartsPage — React migration of src/app/pages/charts/amCharts.
 *
 * Replaces charts.html + the per-chart controllers (BarChartCtrl, AreaChartCtrl,
 * LineChartCtrl, PieChartCtrl, FunnelChartCtrl, combinedChartCtrl). Each chart is
 * created with the global AmCharts library using the "blur" theme registered by
 * the amCharts module's .config block.
 *
 * baConfig and layoutPaths are injected by the AngularJS bridge directive
 * (never read from window) and passed in as props.
 */
import React, { useEffect, useRef } from 'react';
import { Panel } from '../components/Panel';

function buildBarConfig(layoutColors, pathToImages) {
  return {
    type: 'serial',
    theme: 'blur',
    ...
    valueAxes: [
      { axisAlpha: 0, position: 'left', title: 'Visitors from country', gridAlpha: 0.5, gridColor: layoutColors.border }
    ],
    startDuration: 1,
    graphs: [
      {
...
function buildAreaConfig(layoutColors, pathToImages) {
  return {
    type: 'serial',
    theme: 'blur',
    color: layoutColors.defaultText,
    ...
    valueAxes: [
      { duration: 'mm', durationUnits: { hh: 'h ', mm: 'min' }, gridAlpha: 0.5, gridColor: layoutColors.border }
    ],
    graphs: [
      {
        bullet: 'square',
...
function buildLineConfig(layoutColors, pathToImages) {
  return {
    type: 'serial',
    theme: 'blur',
    color: layoutColors.defaultText,
    ...
    valueAxes: [
      { axisAlpha: 0, position: 'left', gridAlpha: 0.5, gridColor: layoutColors.border }
    ],
    graphs: [
      {
        id: 'g1',
...
function buildPieConfig(layoutColors, pathToImages) {
  return {
    type: 'pie',
    startDuration: 0,
    theme: 'blur',
...
function buildCombinedConfig(layoutColors, pathToImages) {
  return {
    type: 'serial',
    theme: 'none',
    color: layoutColors.defaultText,
...
export function AmChartsPage({ baConfig, layoutPaths }) {
  ...
  var pathToImages = (layoutPaths && layoutPaths.images && layoutPaths.images.amChart) || '';
  ...
  useEffect(function () {
    ...
    var barChart = AmCharts.makeChart('barChart', buildBarConfig(layoutColors, pathToImages));
    made.push(barChart);

    var areaChart = AmCharts.makeChart('areaChart', buildAreaConfig(layoutColors, pathToImages));
    areaChart.addListener('dataUpdated', function () {
      areaChart.zoomToDates(new Date(2012, 0, 3), new Date(2012, 0, 11));
    });
    made.push(areaChart);

    var lineChart = AmCharts.makeChart('lineChart', buildLineConfig(layoutColors, pathToImages));
    ...
    var pieChart = AmCharts.makeChart('pieChart', buildPieConfig(layoutColors, pathToImages));
    ...
    made.push(combinedChart);

    chartsRef.current = made;

    return function () {
      chartsRef.current.forEach(function (chart) {
      ...
      <div className="row">
        <div className="col-lg-4 col-md-6">
          <Panel title="Bar Chart" panelClass="with-scroll">
            <div id="barChart" className="admin-chart"></div>
          </Panel>
        </div>
        <div className="col-lg-4 col-md-6">
          <Panel title="Area Chart" panelClass="with-scroll">
            <div id="areaChart" className="admin-chart"></div>
          </Panel>
        </div>
        <div className="col-lg-4 col-md-12">
          <Panel title="Line Chart" panelClass="with-scroll">
            <div id="lineChart" className="admin-chart"></div>
          </Panel>
        </div>
      </div>
      ...
      <div className="row">
        <div className="col-md-6">
          <Panel title="Pie Chart" panelClass="with-scroll">
            <div id="pieChart" className="admin-chart"></div>
          </Panel>
        </div>
        <div className="col-md-6">
          <Panel title="Funnel Chart" panelClass="with-scroll">
            <div id="funnelChart" className="admin-chart"></div>
          </Panel>
        </div>
      </div>
      ...
      <div className="row">
        <div className="col-md-12">
          <Panel title="Combined bullet/column and line graphs with multiple value axes" panelClass="with-scroll">
            <div id="zoomAxisChart" className="admin-chart"></div>
          </Panel>
        </div>
      </div>
    </div>
