...
/**
 * Line Maps page (amCharts makeChart map) — migrated from MapLinesPageCtrl.
...
export function MapLinesPage({ baConfig, layoutPaths }) {
  ...
  useEffect(function () {
    ...
    function init() {
      ...
      if (!AmCharts || !AmCharts.themes || !AmCharts.themes.blur || !AmCharts.maps || !AmCharts.maps.worldLow) {
        setTimeout(init, 100);
        return;
      }
      ...
      // svg path for plane icon
          ...
          setTimeout(function() {
            if (cancelled) return;
            mapInstance = AmCharts.makeChart( 'map-lines', {
              type: 'map',
              theme: 'blur',
              dataProvider: {
                map: 'worldLow',
    ...
    return function () {
      cancelled = true;
      try { if (mapInstance && mapInstance.clear) mapInstance.clear(); } catch (e) { /* noop */ }
