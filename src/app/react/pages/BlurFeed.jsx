/**
 * BlurFeed — React port of the AngularJS blur-feed directive
 * (BlurFeedCtrl + blurFeed.html). The "Feed" widget.
 *
 * Clicking a message toggles its expanded state, revealing the timestamp and
 * (for messages with a preview) the preview image.
 */
...
export function BlurFeed({ layoutPaths }) {
  var [feed, setFeed] = useState(function () {
    return FEED.map(function (message) {
      return Object.assign({}, message, { expanded: false });
    });
  });

  var expandMessage = function (index) {
    setFeed(function (prev) {
      return prev.map(function (message, i) {
        if (i !== index) {
          return message;
        }
        return Object.assign({}, message, { expanded: !message.expanded });
      });
    });
  };
    ...
    <blur-feed>
      <div className="feed-messages-container">
        {feed.map(function (message, i) {
          return (
            <div className="feed-message" key={i} onClick={function () { expandMessage(i); }}>
              ...
              <div className="text-block text-message">
                ...
                <div className={'message-content line-clamp' + (!message.expanded ? ' line-clamp-2' : '')}>
                  {message.preview ? <span>{message.header} </span> : null}{message.text}
                </div>
                {message.preview
                  ? (
                    <div className="preview" style={{ display: message.expanded ? '' : 'none' }}>
                      <a href={message.link} target="_blank" rel="noreferrer">
                        <img src={layoutPaths.images.root + message.preview} />
                      </a>
                    </div>
                  )
                  : null}
                <div className="message-time" style={{ display: message.expanded ? '' : 'none' }}>
