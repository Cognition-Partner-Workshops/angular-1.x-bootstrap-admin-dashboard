...
import { ThemeConfigService, type ThemeColors } from '../../../core/services/theme-config.service';

declare const AmCharts: {
  theme: unknown;
  themes: { blur: unknown };
  AmMap: new () => AmMap;
  maps: { worldLow: unknown };
};

interface AmMap {
  addTitle(title: string, size: number): void;
  areasSettings: {
...
export class BubbleComponent implements AfterViewInit, OnDestroy {
  ...
  private initMap(): void {
    if (typeof AmCharts === 'undefined') {
      console.warn('AmCharts is not loaded');
      return;
    }

    const layoutColors = this.themeConfig.colors;
    const layoutPaths = this.themeConfig.layoutPaths;
    const latlong = this.getLatLongData();
    const mapData = this.getMapData(layoutColors);
    ...
    this.map.addTitle('Population of the World in 2011', 14);
    this.map.addTitle('source: Gapminder', 11);
    this.map.areasSettings = {
      unlistedAreasColor: '#000000',
      unlistedAreasAlpha: 0.1
    };
    this.map.imagesSettings.balloonText = '<span style="font-size:14px;"><b>[[title]]</b>: [[value]]</span>';
    this.map.pathToImages = layoutPaths.images.amMap;
    ...
    this.map.write('map-bubbles');
  }

  private getLatLongData(): Record<string, LatLong> {
    const latlong: Record<string, LatLong> = {};
  ...
  private getMapData(layoutColors: ThemeColors): MapDataItem[] {
    return [
