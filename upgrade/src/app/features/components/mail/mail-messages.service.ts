...
export type MailTag = 'friend' | 'study' | 'work' | 'family';
...
export class MailMessagesService {
  private messages: MailMessage[] = ([
    {
      ...
      tag: 'friend' as MailTag as MailTag,
      labels: ['inbox']
    },
    {
      id: '4563fdfvd',
  ...
  ] as MailMessage[]).sort((a, b) => {
    if (a.date > b.date) return -1;
    if (a.date < b.date) return 1;
    return 0;
  });

  private tabs: MailTab[] = [
  ...
  getMessagesByLabel(label: string): MailMessage[] {
    return this.messages.filter(m => m.labels.includes(label));
  }
