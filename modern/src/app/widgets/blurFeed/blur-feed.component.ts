...
type FeedType = 'text-message' | 'video-message' | 'image-message' | 'geo-message';
...
/**
 * Modern standalone port of the legacy AngularJS dashboard "Feed" widget
 * (`blurFeed`). Renders a scrollable list of feed messages; clicking a message
 * toggles its expanded state (line-clamp off, plus preview image and times).
 *
 * Renders only the widget's inner content — the surrounding panel is applied by
 * the integration step via `app-ba-panel`.
 */
@Component({
...
export class BlurFeedComponent {
  ...
  /** Class selecting the author's avatar background image (see SCSS). */
  avatarClass(message: FeedMessage): string {
    return 'avatar--' + message.author;
  }

  /** Class selecting the preview background image (see SCSS). */
  previewClass(message: FeedMessage): string {
    if (!message.preview) {
      return '';
    }
    const file = message.preview.split('/').pop() ?? '';
    return 'preview--' + file.replace(/\.[^.]+$/, '');
  }
}
